...
> **⚠️ STATUS UPDATE (after the advisor's answers).**
> The notes below were written *before* the advisor answered, and some framing is now
...
> - The "two CSVs" were **Ben's watch data (sleep + light)** — a different project, not our CGM. We don't need them; Task 3 (understand TSFM input format) is done against the **ICML `representations-in-tsfms` repo**, which the advisor reproduced.
> - Scope is now **1 channel (CGM only), single-target, 3 separate models (Grids/Symbols/Prices), raw-data/TSFM arm only.**
> - The real data + the 3 targets live in the advisor's **`diabetes-fitbit`** repo; its feature-engineering arm (Mack's) got **no predictive power**, which calibrates expectations.
> - A runnable pipeline (both arms) now exists in **`cgm_tsfm/`**.
>
> **What's still valid below:** the detailed analysis of *Ben's code* (Task 2 / Task 4 mechanics) — the channel hardcode, the regression head, the `/365` normalization — all confirmed against the current code and reused in `cgm_tsfm/ben_adapter/`.

*Brian's notes on the tasks the advisor assigned (see the task-assignment screenshot). Task 1 (dev env on compute2) is deferred per Brian's own instruction — everything below covers tasks 2–4. Written 2026-07-01.*
...
Ben's project predicts preterm birth from actigraphy (wrist motion + light) using, among other models, two frozen pretrained time-series foundation models — Amazon's **Chronos-Bolt** and Google's **TimesFM** — as feature encoders, with a small trainable transformer + classification head stacked on top. This is the direct architectural template the advisor wants applied to CGM (glucose) data instead of actigraphy data.
...
**The two CSV files mentioned in the task aren't on this machine.** I searched Desktop, Downloads, Documents, and the project folders — nothing matching CGM/glucose/Dexcom data turned up (only unrelated old coursework CSVs from a 2022 folder). This needs to come from the advisor or wherever the study's data is stored — logged in `docs/04_OPEN_QUESTIONS.md`.
...
3. **Single- vs. multi-target regression is an open design question, not just an implementation detail.** The K01 proposal's own Aim 1 analysis plan (p.7) calls for predicting **four separate continuous outcomes**: Mean Symbols RT, Mean Grids Error Score, SD of Symbols RT, SD of Grids Error Score. Ben's current code assumes exactly one regression target (`num_classes=1`, then squeezed to a plain scalar). Supporting multiple simultaneous targets would mean *not* squeezing, keeping `logits` as `(B, N)`, and computing per-target loss/metrics — a bigger change than just flipping a flag. Worth confirming with the advisor whether the immediate goal is one target at a time (simplest, reuses Ben's pattern almost as-is) or joint multi-target regression (see questions log).
