...
- [x] **Markdown results auto-dump**: both `run_sweep` and `run_headtohead` write writeup-ready tables to `results/*.md` (metadata header, per-target tables, verdicts, synthetic caveat). Remaining: figures, and folding the real-data numbers into the K01's "raw-data vs feature-engineering" comparison.
