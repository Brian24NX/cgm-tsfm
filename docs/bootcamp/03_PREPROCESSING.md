...
**How much does it matter?** We do not know, and you should say so. Liuyi's own position is *"As far as I know, the CGM monitoring is continuous, i.e., always on. So, from my understanding, there is no need for imputation"* (note his hedges) — under which reading there are few real gaps and the effect is small. Against that, the original Cohort2 file had 44 of 240 rows with **entirely** empty glucose, and the `Updated_` file we use has only 1, so something filled those in and we have not seen the code that did it (see `01_FACT_SHEET.md` section B). **Do not claim the timing is clean.**
